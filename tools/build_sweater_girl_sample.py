...
大学の門・書架・机・階段・ベンチなどが入っているカット（poses.json で
has_bg が true のグループ）に辞書の背景を重ねると、線が二重になって
「背景が透けている」ように見える。
...
PAGES = [
    ...
    {"layout_id": "3段4コマ1", "panels": [
        P("C466", None, [N("先輩とは、大学で知り合った。")]),
        P("C421", None, [C("今日、ひま？")]),
        P("C470", None, [C("ひまだけど")]),
        P("C476", None, [C("じゃあ、道場つきあって")], sfx=[("パァァ", 2)]),
    ]},
    # p02 道場。刀を構える
    ...
    {"layout_id": "2段2コマ1", "panels": [
        ...
        P("C480", None, [N("うちの先輩は、強い。"), N("転ぶけど。")], bg_y=0.55),
    ]},
]
